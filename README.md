...
基于Hadoop+GoldenGate的Oracle到HBase实时数据同步项目，实现电信计费数据的实时分析，包含配置文件、部署脚本与实践文档。
# 运营商用户行为实时数据分析系统
...
```mermaid
flowchart LR
    subgraph 源端MySQL生产集群【我核心负责运维】
        direction LR
        M[MySQL主库<br/>存储电信计费原始数据] -->|GTID主从复制| S[MySQL从库<br/>灾备只读节点]
    end

    subgraph 数据同步层【团队资深DBA负责】
        direction LR
        M -->|Binlog实时变更捕获| CDC[商业CDC同步工具<br/>全量+增量数据同步]
        CDC --> R[Replicat写入进程<br/>【我负责对接适配】]
    end

    subgraph 目标端K8s大数据集群【我核心负责运维】
        R --> H[HBase列式存储<br/>海量数据高并发读写]
        H <--> Hive[Hive数据仓库<br/>标准SQL离线分析]
    end
    ...
    style Hive fill:#f3e5f5,stroke:#4a148c,stroke-width:2px
```

## 核心技术栈
| 技术分类 | 组件与版本 |
|----------|------------|
| 核心数据库 | MySQL 5.7（GTID主从复制）、HBase 1.2.1 |
| 数据同步 | Oracle GoldenGate（目标端适配）、Sqoop 1.4.6 |
| 大数据计算 | Hadoop 2.6.4、Hive 2.0.0、Zookeeper 3.4.8 |
| 容器编排 | Kubernetes 1.20+ |
| 运维自动化 | Shell、Crontab、钉钉/邮件告警 |
| 操作系统 | CentOS 7 x86_64 |

## 我负责的核心工作
1.  **MySQL生产集群全生命周期运维**：负责源端MySQL计费库的日常运维、慢查询优化、性能基线管理、备份恢复策略落地，保障核心交易业务7*24小时稳定运行，数据库可用性达99.99%。
2.  **MySQL主从复制自动化监控体系**：针对MySQL一主一从架构，自研Shell自动化监控脚本，分钟级检测IO/SQL线程状态、主从同步延迟、主从数据一致性，异常时自动触发邮件+钉钉双通道告警，将人工巡检成本降低80%，故障发现时间从1小时缩短至1分钟。
3.  **目标端数据同步链路适配与落地**：负责GoldenGate目标端Replicat进程的配置、HBase表结构对接、异构数据字段映射适配，完成CDC同步链路与HBase存储的打通，保障增量数据毫秒级写入HBase，同步延迟稳定控制在200ms以内。
4.  **K8s大数据集群运维管理**：负责部署在Kubernetes上的Hadoop/HBase/Hive集群的日常运维、组件状态巡检、资源调度优化，处理Pod异常、存储卷挂载、网络连通性等常见故障，保障大数据平台稳定运行。
5.  **Hive与HBase集成与性能优化**：完成Hive外部表与HBase的字段映射开发，实现业务人员通过标准SQL即可查询海量实时数据；针对高频分析场景做SQL优化、数据倾斜治理，将全量用户分析查询耗时从2小时优化至20分钟，查询效率提升80%。
6.  **全链路故障排查与治理**：独立排查解决目标端同步写入、MySQL主从复制、HBase连接、Hive查询等核心生产故障，沉淀标准化故障排查手册，将故障平均恢复时间从4小时缩短至30分钟。

## 项目成果
- ✅ 完成源端到目标端的**毫秒级实时数据同步**，替代原有T+1离线同步方案，满足业务实时分析需求
- ✅ 完成16万+条历史计费数据的批量初始化导入，数据一致性校验通过率100%，无数据丢失
- ✅ 为业务提供标准化SQL分析接口，降低大数据使用门槛，支撑15+业务运营报表的日常查询
- ✅ 全链路稳定性达99.9%，上线后无计划外停机，支撑1200万+电信用户的行为分析需求
- ✅ 自动化运维体系覆盖全链路，人工运维成本降低80%，故障处理效率提升87.5%
...
│   └── hive_sql_optimize.sh  # Hive高频查询性能优化脚本
├── sql/                   # SQL脚本
│   ├── create_bill_table.sql # 源端计费表结构创建脚本
